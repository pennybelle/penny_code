...
class AutoDeleteCog(commands.Cog):
    ...
    async def _cut(self, channel, start_id, stop_id, limit=1000):
        channel_id = self.bot.get_channel(int(ID))
        if start_id == "*":
            start_id = await channel_id.fetch_message(channel_id.last_message_id)
            is_deleting = True
        else:
    ...
    async def on_message(self, message):
        ...
        if len(blacklist_word & message_words) > 0 or any(
                substring in message_text for substring in blacklist_contains) or any(
                re.search(cur_regex, message_text) is not None for cur_regex in blacklist_regex):
